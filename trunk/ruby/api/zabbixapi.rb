...
class ZbxAPI
  ...
  public
    ...
    def debug(level,variable="",message=nil)
      if level<=@debug_level
        ...
        puts "**Debug(#{level})** #{debug_func}: #{strval}"
      end
    end
  ...
  def do_request(json_obj)
    ...
    http.start do |http|
      ...
      debug(8,"Response Body: #{response.body}")
    end
...
class ZbxAPI_Host < ZbxAPI_Sub
  ...
  def create(options={})
    ...
    obj=do_request(json_obj('host.create',options))
      ...
      if e.message["code"]==-32500
        return {:class => :host, :result => 0}
      else
        raise e
      end
    end
  ...
  def add(options={})
    puts "WARNING API Function Host.create will is deprecated and will be removed in the future without further warning"
    create(options)
  end
...
class ZbxAPI_Item < ZbxAPI_Sub
  ...
  def add(options)
    puts "WARNING API Function Item.add will is deprecated and will be removed in the future without further warning"
    create(options)
  end
...
class ZbxAPI_HostGroup < ZbxAPI_Sub
  ...
  # alias function for code written against 1.0 API
  def add(options={})
    puts "WARNING API Function HostGroup.add will is deprecated and will be removed in the future without further warning"
    create(options)
  end
  ...
  def getId(options={})
    ...
    begin
    obj=do_request(json_obj('hostgroup.getId',options))
    rescue ZbxAPI_GeneralError => e
      if e.message["code"]==-32602
        return 0
      else
