...
class OutputPrinter
  ...
  def pause? (lines=1)
    if @interactive and @sheight>0 and (@lines>-1) then
      @lines += lines
      if @lines>=(@sheight-1) then
...
#            putc('.')
  ...
  def print_array(dataset,cols)
    ...
    if results.length==0
      ...
      if cols.nil? then
        case type
          when :user : header=["userid","alias"]
          when :host : header=["hostid","host"]
          when :item : header=["itemid","description","key_"]
          when :hostgroup : header=["groupid","name"]
        end
...
#    p dataset[:result].class
      if dataset[:result].class==Array then
        print_array(dataset,cols)
      else
        print_hash(dataset,cols)
      end
